...
- **`app/calendar.py`** — trading-calendar utilities (`is_trading_day`, `is_trading_time`, `now_sh`, `today_sh`). All time judgments use fixed UTC+8 (Shanghai time) to stay consistent with APScheduler's `timezone='Asia/Shanghai'` regardless of server system TZ.
- **`app/qmt.py`** — QMT connection module. Owns the singleton `XtQuantTrader` and `StockAccount` (via `connect()` / `get_account()` / `disconnect()`). Also re-exports `xtdata` and `xtconstant`. All other modules import xtquant symbols through this module — never `from xtquant.xxx import ...` directly.
...
- **`app/trade.py`** — instrument resolution and order placement. Provides `append_suffix` (6-digit code → suffixed code, covers A股/科创板/创业板301/ETF/LOF/可转债/逆回购), `build_name_cache`, `resolve_code`, `place_order`, `check_order`, and `main(argv)` for the `trade` subcommand.
- **`app/repo.py`** — treasury bond reverse repo (国债逆回购) scheduled task. At 14:58 on each trading day, compares the 1-day repo yield of Shanghai (`204001.SH`) vs Shenzhen (`131810.SZ`), sells the higher-rate market using all available cash at counterparty-best price. Entry point for scheduler: `scheduled_repo()`. Business logic: `run_repo()`. (No CLI subcommand — use `serve` or call `run_repo()` directly to test.)
- **`app/watch.py`** — price-monitoring feature with DingTalk notification. Provides CRUD (`add_watch`, `list_watches`, `delete_watch`, `reset_watch`, `update_watch`, `resolve_watch`), the per-minute `check_watches()` (batches DB commits and sends notifications only after commit succeeds to prevent duplicate alerts), and the scheduler entry `scheduled_watch()`. CLI: `watch add/list/update/delete/reset/now`.
