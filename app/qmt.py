...
实盘/模拟：connect(mode=...) 传入 'live' 或 'sim' 选择连接哪个终端，
两者复用全部下单/行情逻辑，仅 mini_path/account_id 不同（来自 config）。
单例绑定首次 connect 的 mode；同一进程不混用（需要同时连两个终端时再上子类）。
"""
import logging
import random
...
class QmtClient:
    ...
    _trader = None
    _account = None
    _mode = None  # 已连接的后端：'live' | 'sim'，供断言单例不跨 mode 复用
    ...
    _MODE_CONFIG = {
        'live': lambda: (config.mini_path, config.account_id),
        'sim': lambda: (config.sim_mini_path, config.sim_account_id),
    }

    @classmethod
    def connect(cls, mode='live'):
        """连接 mini QMT，建立共享的 xt_trader + StockAccount 单例。

        mode: 'live' 实盘终端 / 'sim' 模拟终端（券商模拟撮合，不产生真实成交）。
        单例绑定首次调用的 mode；以不同 mode 再次调用会抛错，避免串号。
        """
        if cls._trader is not None:
            if cls._mode != mode:
                raise RuntimeError(
                    f'QmtClient 已以 mode={cls._mode!r} 连接，不能再用 mode={mode!r} 重连'
                )
            return cls._trader
        getter = cls._MODE_CONFIG.get(mode)
        if getter is None:
            raise ValueError(f'未知 mode: {mode!r}（应为 live/sim）')
        mini_path, account_id = getter()
        if not mini_path or not account_id:
            raise RuntimeError(
                f'mode={mode!r} 的 mini_path/account_id 未配置（检查 config.toml 的 [qmt] 或 [qmt.sim]）'
            )
        ...
        cls._account = acc
        cls._mode = mode
        tag = '实盘' if mode == 'live' else '模拟'
        logger.info(f'连接成功[{tag}], 订阅账户: {account_id}')
        return cls._trader
    ...
    @classmethod
    def get_trader(cls):
        """返回共享的 xt_trader 单例；若尚未 connect() 则以实盘模式建立连接。"""
    ...
    @classmethod
    def get_account(cls):
        """返回共享的 StockAccount 单例；若尚未 connect() 则以实盘模式建立连接。"""
